...
function create_accountaction($connect,$name,$email,$password,$ucategory,$username){

	$options = [
		'cost' => 12,];
	$hash_pass = password_hash("$password", PASSWORD_BCRYPT, $options);
	$sql = "INSERT INTO tblaccount VALUES ('','$name', '$username','$hash_pass','$email','Inactive', '$ucategory','No','','')";
	$result = $connect->query($sql);
	if ($result === true) {
		return 1;
	} else {
...
function get_subscribe($connect){
	//ORDER BY deadline ASC
	$sql = "SELECT * FROM tblaccount WHERE ucategory='User' AND subcribe='YES'";
	$result = $connect->query($sql);
		return $result;
}
function get_unsubscribe($connect){
	//ORDER BY deadline ASC
	$sql = "SELECT * FROM tblaccount WHERE ucategory='User' AND subcribe='No'";
	$result = $connect->query($sql);
		return $result;
}
function get_alluser($connect){
	//ORDER BY deadline ASC
	$sql = "SELECT * FROM tblaccount WHERE ucategory='User'";
	$result = $connect->query($sql);
		return $result;
}
?>
